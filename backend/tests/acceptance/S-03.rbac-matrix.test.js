...
describe('S-03 ma trận quyền Sprint 1 (PO đã xác nhận) trên API thật', () => {
  ...
  before(async () => {
    ...
    const st = await request(app).post('/api/stations').set('Cookie', users.ADMIN.cookie)
      .send({ name: 'Trạm A', address: 'Hà Nội', owner_id: users.STATION_OWNER.id });
  ...
  const cases = [
    ...
    { name: 'POST /api/stations', allowed: WRITE, ok: 201, call: (r, t, context) => r.post('/api/stations').send({ name: `T${++seq}`, address: 'HN', ...(context.role === 'ADMIN' ? { owner_id: users.STATION_OWNER.id } : {}) }) },
    ...
    { name: 'GET /api/admin/station-owners', allowed: ['ADMIN'], ok: 200, call: (r) => r.get('/api/admin/station-owners') },
  ];

  for (const c of cases) {
    it(`${c.name}: chỉ ${c.allowed.join(', ')}; vai trò khác → 403; không phiên → 401`, async () => {
      for (const role of ROLES) {
        const req = c.call(request(app), role === 'STATION_OWNER' ? own : shared, { role }).set('Cookie', users[role].cookie);
        const res = await req;
