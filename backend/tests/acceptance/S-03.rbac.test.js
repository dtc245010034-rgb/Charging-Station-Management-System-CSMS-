...
describe('S-03 lọc sở hữu ở tầng truy vấn (2 chủ trạm A/B)', () => {
  ...
  it('S-03: Admin phân owner hợp lệ, owner khác không được tự đổi owner', async () => {
    const created = await as(u.admin).post('/api/stations', {
      name: 'Trạm Admin phân', address: 'HN', owner_id: u.B.id, status: 'MAINTENANCE',
    });
    assert.strictEqual(created.status, 201);
    assert.strictEqual(String(created.body.owner_id), String(u.B.id));
    assert.strictEqual(created.body.is_active, true);

    const denied = await as(u.A).patch(`/api/stations/${created.body.id}`, { owner_id: u.A.id });
    assert.strictEqual(denied.status, 403);

    const row = (await query('SELECT owner_id, is_active FROM stations WHERE id = $1', [created.body.id])).rows[0];
    assert.strictEqual(String(row.owner_id), String(u.B.id));
    assert.strictEqual(row.is_active, true);
  });

  it('S-03: status INACTIVE đồng bộ is_active=false', async () => {
    const created = await as(u.A).post('/api/stations', { name: 'Trạm tắt', address: 'HN', status: 'INACTIVE' });
    assert.strictEqual(created.status, 201);
    assert.strictEqual(created.body.is_active, false);
    const updated = await as(u.A).patch(`/api/stations/${created.body.id}`, { status: 'ACTIVE' });
    assert.strictEqual(updated.status, 200);
    assert.strictEqual(updated.body.is_active, true);
  });
