...
const insert = (s, ownerId) => prepare('INSERT INTO stations (name, address, latitude, longitude, status, is_active, owner_id) VALUES (?, ?, ?, ?, ?, ?, ?)')
  .run(s.name, s.address, s.latitude ?? null, s.longitude ?? null, s.status, s.status !== 'INACTIVE', ownerId);

const UPDATABLE = ['name', 'address', 'latitude', 'longitude', 'status', 'owner_id'];
const update = (id, fields) => {
  const keys = UPDATABLE.filter((key) => fields[key] !== undefined);
  const assignments = keys.map((key) => key === 'status' ? 'status = ?, is_active = ?' : `${key} = ?`);
  const values = keys.flatMap((key) => key === 'status' ? [fields[key], fields[key] !== 'INACTIVE'] : [fields[key]]);
  return prepare(`UPDATE stations SET ${assignments.join(', ')}, updated_at = CURRENT_TIMESTAMP WHERE id = ?`)
    .run(...values, id);
};
